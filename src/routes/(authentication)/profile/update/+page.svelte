<script lang="ts">
    ...
    async function updateUser() {
      fetch("https://monkeybackend.rohanj.dev/api/login/updateUser", {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Accept': 'application/json',
...
<div class="update-profile-container">
    {#if authenticated}
      <h1 class="update-profile-title">Change User</h1>
      <div class="update-profile-field">
        <input bind:value={updatedName} type="text" placeholder={userName} class="input w-full max-w-xs " />
        <input type="button" value="Update" class="btn btn-primary my-4 w-full" on:click={updateUser} />
      </div>
    {:else}
